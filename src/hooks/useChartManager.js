...
export function useChartManager({
  ...
  const handleLoadChart = useCallback(async (chart) => {
    ...
    const stripped = hydrated.map(n => ({ ...n, position: { x: 0, y: 0 } }))
    const normalizedEdges = chart.edges.map(e => e.type === 'step' ? { ...e, type: 'smoothstep' } : e)
    setNodes(applyDagreLayout(stripped, normalizedEdges))
    setEdges(normalizedEdges); setCounter(chart.counter)
